...
const createAcademicSemistar = catchAsync(async (req, res) => {
  ...
  const result =
    await CreateAcademicSemistarService.createAcademicSemistarIntoDB(req.body);
  sendResponse(res, {
...
const getAllAcademicSemisatrt = catchAsync(async (req, res) => {
  const result =
    await CreateAcademicSemistarService.getAllAcademicSemisTarServiceFromDb();
  sendResponse(res, {
    statusCode: HttpStatus.OK,
    success: true,
...
const getASingleAcademicSemistar = catchAsync(async (req, res) => {
  ...
  const result =
    await CreateAcademicSemistarService.getSingleSemistarServiceFromDb(
      semistarId
    );
...
const UpdateAcademicSemistar = catchAsync(async (req, res) => {
  const { semistarId } = req.params;

  const result = await CreateAcademicSemistarService.UpdateSemistartService(
    semistarId,
    req.body
  );
